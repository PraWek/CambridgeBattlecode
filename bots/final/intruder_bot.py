"""A combat BuilderBot role that infiltrates and supplies a forward gunner."""
...
class IntruderBot(CombatBot):
    ...
    def __init__(self, map_width: int, map_height: int) -> None:
        ...
        self.waiting_launch_marker: Position | None = None
        ...
        self.owned_supply_tiles: set[Position] = set()
        self.owned_supply_ores: set[Position] = set()
    ...
    def claims_spawn(controller: Controller) -> bool:
        ...
        return read_spawn_assignment(controller)[0]
    ...
    def wait_for_launcher(self, controller: Controller, current: Position) -> bool:
        """Wait for the launcher to finish scanning and throw this bot."""
        ...
        if current != self.waiting_launcher_origin:
            ...
            self.waiting_launch_marker = None
            self.wall_following = False
            return False
        # A newborn launcher needs several turns to finish its terrain scan.
        if controller.get_current_round() <= self.waiting_launcher_round + LAUNCH_WAIT_ROUNDS:
            return True
        ...
        self.waiting_launcher_origin = None
        self.waiting_launcher_round = None
        if self.waiting_launch_marker is not None and controller.can_destroy(self.waiting_launch_marker):
            controller.destroy(self.waiting_launch_marker)
            self.tile_cache.forget_building(self.waiting_launch_marker)
        self.waiting_launch_marker = None
    ...
    def start_launcher_crossing(
        ...
        for launcher_pos in self.launcher_build_sites(current, landing):
            ...
            if not controller.can_build_launcher(launcher_pos):
                continue
            # Address the throw to the intruder's tile; an economic worker may
            # also be adjacent and must not be launched away from its mine.
            source_code = DIRECTIONS.index(launcher_pos.direction_to(current)) + 1
            marker_value = encode_marker(MARKER_KIND_INTRUDER_LAUNCH, landing, source_code)
            ...
            self.waiting_launch_marker = marker_pos
            return True
        return False
    ...
    def build_forward_gunner(self, controller: Controller, current: Position) -> None:
        ...
        if self.is_protected_transport(site):
    ...
    def is_gunner_site_locally_viable(self, site: Position) -> bool:
        ...
        if site in self.rejected_gunner_sites or self.is_protected_transport(site):
            return False
    ...
    def is_protected_transport(self, pos: Position) -> bool:
        """Only repurpose this intruder's own allied transport, never a mine lane."""
        building = self.known_buildings.get(pos)
        return (
            ...
            and pos not in self.owned_supply_tiles
        )

    def is_available_supply_ore(self, pos: Position) -> bool:
        building = self.known_buildings.get(pos)
        return (
            building is None
            or building[1] != self.team
            or pos in self.owned_supply_ores
        )
    ...
    def clear_walkable_tile(
        ...
        if building_team == self.team:
            if self.is_protected_transport(pos):
                return False
        ...
        if self.supply_plan_candidates is None:
            self.supply_plan_candidates = sorted(
                (
                    ...
                    if env == Environment.ORE_TITANIUM and self.is_available_supply_ore(pos)
    ...
    def is_supply_bridge_landing(self, pos: Position) -> bool:
        ...
        if env is None or env in {Environment.WALL, Environment.ORE_TITANIUM, Environment.ORE_AXIONITE}:
            return False
        building = self.known_buildings.get(pos)
        return not self.is_protected_transport(pos) and (
            building is None or building[0] in _CLEARABLE_WALKABLE_BUILDINGS
        )
    ...
    def known_supply_bridge_crossings(self) -> dict[Position, Position]:
        ...
        return {
            ...
            and not self.is_protected_transport(bridge)
        }
    ...
    def ensure_supply_harvester(self, controller: Controller, current: Position) -> bool:
        ...
        if self.supply_ore is None:
            return False
        if not self.is_available_supply_ore(self.supply_ore):
            self.reset_supply_plan()
            return False
        ...
        self.owned_supply_ores.add(self.supply_ore)
        self.tile_cache.remember_building(
            self.supply_ore,
            harvester_id,
    ...
    def build_supply_tile(self, controller: Controller, tile: Position) -> bool:
        ...
        if self.is_protected_transport(tile):
            self.reset_supply_plan()
            return False
        ...
        if wants_bridge:
            ...
            self.owned_supply_tiles.add(tile)
