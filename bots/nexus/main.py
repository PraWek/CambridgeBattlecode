    ...
    def try_spawn_builder(
        ...
        if self.spawned_builders == 1:
            ...
            if titanium < builder_cost + SECOND_BUILDER_RESERVE:
                _logger.log_info(
                    ct,
                    f"Cannot spawn second builder: not enough titanium "
                    f"({titanium} < {builder_cost + SECOND_BUILDER_RESERVE}).",
                )
                return
        ...
        if self.spawned_builders == 2:
            ...
            if titanium < builder_cost + THIRD_BUILDER_RESERVE:
                _logger.log_info(
                    ct,
                    f"Cannot spawn third builder: not enough titanium "
                    f"({titanium} < {builder_cost + THIRD_BUILDER_RESERVE}).",
                )
                return
        ...
        if self.spawned_builders == 3:
            ...
            if titanium < builder_cost + FOURTH_BUILDER_RESERVE:
                _logger.log_info(
                    ct,
                    f"Cannot spawn fourth builder: not enough titanium "
                    f"({titanium} < {builder_cost + FOURTH_BUILDER_RESERVE}).",
                )
                return
